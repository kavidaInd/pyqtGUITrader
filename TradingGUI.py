...
class TradingGUI(QMainWindow):
    ...
    def _show_live_upgrade_dialog(self):
        """
        Show the LiveTradingUpgradeDialog when a free/trial user clicks Start
        in LIVE mode.  Handles two outcomes:

          1. User activates a paid license  → proceed to start the engine
          2. User clicks "Continue in Paper Mode" → switch mode to PAPER and
             start the engine automatically so they keep going with no friction
        """
        try:
            ...
            def _on_paper():
                ...
                try:
                    if self.trading_mode_setting:
                        ...
                        self._update_mode_display()
                        logger.info("Switched to PAPER mode at user request after upgrade prompt")
                    # Start immediately — no second click needed
                    self._start_app()
                except Exception as e:
                    logger.error(f"[_show_live_upgrade_dialog._on_paper] {e}", exc_info=True)
