...
========================
Zerodha KiteConnect implementation of BaseBroker with token expiry handling.
...
class ZerodhaBroker(BaseBroker):
    """
    Zerodha KiteConnect broker implementation with token expiry handling.
        ...
        try:
            ...
            if access_token:
                ...
                if self.is_token_expired:
                    logger.warning("Zerodha token is expired at load time")
            else:
    ...
    def _on_token_recovered(self) -> None:
        ...
        """
        logger.info("[ZerodhaBroker] Token recovered, reloading from DB")
        ...
        if access_token:
            ...
            self.state.token = access_token
            self._token_expiry = self._parse_token_expiry()
            logger.info("[ZerodhaBroker] Kite client re-initialized with new token")
    ...
    def _call(self, func: Callable, context: str = "",
        ...
        for attempt in range(max_retries):
            try:
                ...
                # _handle_token_expired raises TokenExpiredError
